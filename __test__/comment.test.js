...
const queryCreateComment = {
  query: `mutation Mutation($recipeId: ID, $message: String) {
    createComment(recipeId: $recipeId, message: $message) {
      message
    }
  }`,
};
...
const queryDeleteComment = {
  query: `mutation Mutation($commentId: ID) {
...
describe("user testing register", () => {
  ...
  it("should be success createComment", async () => {
    // send our request to the url of the test server
    queryCreateComment.variables = {
      message: "Hai ini enak sekali",
    };
    const response = await request(urlTest)
      .post("/")
      .send(queryCreateComment)
      .set({ access_token: createToken({ id: 1, email: "marsh@potao.com" }) });
    // console.log(response.body, "<<<");
    expect(response.body.errors).toBeUndefined();
    expect(response.body.data).toBeInstanceOf(Object);
    expect(response.body.data).toHaveProperty("createComment");
  });

  it("should be failed createComment due to invalid token", async () => {
    // send our request to the url of the test server
    queryCreateComment.variables = {
      message: "Hai ini enak sekali",
    };
  ...
  it("should be failed createComment due to user don't have any access_token", async () => {
    // send our request to the url of the test server
    queryCreateComment.variables = {
      message: "Hai ini enak sekali",
    };
    const response = await request(urlTest).post("/").send(queryCreateComment);
    // console.log(response.body.errors, "<<<");
    expect(response.body.errors).toBeInstanceOf(Array);
    expect(response.body.errors[0]).toHaveProperty("message");
  ...
  it("should be failed deleteComment due to user not authorized", async () => {
    // send our request to the url of the test server
    queryDeleteComment.variables = {
      commentId: "1",
    };
    const response = await request(urlTest)
      .post("/")
      .send(queryDeleteComment)
  ...
  it("should be failed deleteComment due to data not found", async () => {
    // send our request to the url of the test server
    queryDeleteComment.variables = {
      commentId: "14",
    };
  ...
  it("should be failed deleteComment due to user don't have any access_token", async () => {
    // send our request to the url of the test server
    queryDeleteComment.variables = {
      commentId: "1",
    };
    const response = await request(urlTest).post("/").send(queryDeleteComment);
    // console.log(response.body.errors, "<<<");
    expect(response.body.errors).toBeInstanceOf(Array);
    expect(response.body.errors[0]).toHaveProperty("message");
    expect(response.body.errors[0].message).toEqual(
  ...
  it("should be failed deleteComment due to invalid access_token", async () => {
    // send our request to the url of the test server
    queryDeleteComment.variables = {
      commentId: "1",
    };
  ...
  it("should be success deleteComment", async () => {
    // send our request to the url of the test server
    queryDeleteComment.variables = {
      commentId: "1",
    };
    ...
    expect(response.body.errors).toBeUndefined();
    expect(response.body.data).toBeInstanceOf(Object);
    expect(response.body.data).toHaveProperty("deleteComment");
  });
});
